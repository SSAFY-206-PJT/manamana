...
def recommand_webtoon(data: list):
    ...
    user_webtoon_score = df.pivot_table('score', index = 'userId', columns='webtoonId').fillna(0) # vaule: score, column: webtoon, row: user
...
def webtoons_recommand_top10(target_webtoon_id: int) -> list:
    ...
    sort_by_corr = sorted(list(zip(webtoon_title_list, corr_coffey_hands)), key=lambda x: x[1], reverse=True)
